	...
	if [[ "${file##*.}" == "mp4" || "${file##*.}" == "mkv" ]]; then
		...
		if [[ $video_duration != $m3u8_duration && ! -f "${file}.aria2"  ]]; then
			...
			if  ! cat ${m3u8file} | grep "#EXT-X-ENDLIST" &>/dev/null ; then
				...
				ffmpeg -i "${file}" -c copy -bsf:v h264_mp4toannexb -hls_time 6 -hls_list_size 0 -hls_segment_filename "${tmp_m3u8dir}/%04d.ts" "${m3u8file}" > /dev/null 2>&1
				mv ${tmp_m3u8dir} ${m3u8dir}
				echo "${file}  完成切片"
			fi
